...
async def createUser(user: users_schemas.UserCreate = Body(...), db: Session = Depends(database.get_db)):
    ...
    try: 
        ...
        db.rollback()
        await validators.Validator_Functions.general_error("User with email or username already exists", status.HTTP_409_CONFLICT)
    ...
    try:
        ...
        await validators.Validator_Functions.general_error("Email sending failed", status.HTTP_400_BAD_REQUEST)
...
async def confirmResetEmail(update_info: users_schemas.EmailVerify = Body(...), db: Session = Depends(database.get_db)):
    ...
    try:
        ...
        db.rollback()
        await validators.Validator_Functions.general_error("Database update failed", status.HTTP_409_CONFLICT)
...
async def resetPassword(
    ...
    try: 
        ...
        await validators.Validator_Functions.general_error("Reset code has expired", status.HTTP_403_FORBIDDEN)
